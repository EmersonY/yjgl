	...
	renderRow: function(target, fields, frozen, rowIndex, rowData){
		...
		for(var i=0; i<fields.length; i++){
			...
			if (col){
				...
				} else if(col.field == '_expander'){
					cc.push('class="datagrid-cell-rownumber');
				} else{
...
//		state.ss.bindDetailEvents = true;
	...
	onBeforeRender: function(target){
		...
		if (!hasExpander){
			...
			var td = $('<td rowspan="'+opts.frozenColumns.length+'"><div class="datagrid-header-expander" style="width:35px;"></div></td>');
			if ($('tr',t).length == 0){
